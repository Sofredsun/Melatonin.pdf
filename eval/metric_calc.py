...
def main() -> None:
    ...
    df_dataset['doi'] = df_dataset['doi'].apply(lambda x: str(x).lower())
    df_result['doi'] = df_result['doi'].apply(lambda x: str(x).lower())

    df_metrics = empty_metrics(cols)
    access_articles = df_dataset['doi'].unique()
    ...
    for article in access_articles:
        df_dataset_doi = df_dataset.loc[df_dataset['doi'] == article][cols]
        df_result_doi = df_result.loc[df_result['doi'] == article][cols]
